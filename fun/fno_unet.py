...
class FNOUNet(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, depth: int = 4, base_channels: int = 64, kbase1: int = 128, kbase2: int = 128) -> None:
        super().__init__()
        ...
        self.__up_blocks = nn.ModuleList(
            ...
            + [
                nn.Sequential(
                    Residual_Layer(SpectralConv2d(base_channels, base_channels, ksize1=kbase1, ksize2=kbase2), nn.ReLU()),
                    Residual_Layer(SpectralConv2d(base_channels, base_channels, ksize1=kbase1, ksize2=kbase2), nn.ReLU()),
                    nn.Conv2d(base_channels, in_channels, kernel_size=1, bias=False),
                )
            ]
        )
