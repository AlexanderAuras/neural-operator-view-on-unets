...
class MultiDimUNet(UNetBase):
    def __init__(self, in_channels: int, out_channels: int, depth: int = 4, u_shape: bool = True, use_checkpointing: bool = False) -> None:
    ...
    def __partial_forward(self, x: Tensor) -> tuple[Tensor, list[Tensor], list[Tensor]]:
        ...
        for i in range(self._depth - 1):
            if self._u_shape:
                x, ks = cast(nn.ModuleList, self._downsample_layers)[i](x)
        ...
        if self._u_shape:
            x, ks = cast(nn.ModuleList, self._downsample_layers)[self._depth - 1](x)
            keep_shape.append(ks)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for i in range(self._depth - 1):
            if self._u_shape:
                x = cast(nn.ModuleList, self._upsample_layers)[i](x, keep_shape=keep_shape[-(i + 1)])
            x = x + tmp[-(i + 1)]
            x = self._up_conv_layers[i](x)

        if self._u_shape:
            x = cast(Tensor, TrigonometricResize_2d(shape=orig_shape)(x))
        x = x + tmp[0]
...
def create_conv_layer(parametrization: Literal["spectral", "spatial"], in_channels: int, out_channels: int, ksize1: int, ksize2: int, kernel_size: int) -> nn.Module:
        ...
        self._down_blocks = nn.ModuleList(
            [
                nn.Sequential(
                    ...
                    nn.ReLU(),
                    Residual_Layer(nn.Conv2d(base_channels, base_channels, kernel_size=3, padding=1, padding_mode="circular"), nn.ReLU()),
                )
            ]
            + [
                nn.Sequential(
    ...
    def __init__(
        ...
        self._down_conv_layers = nn.ModuleList(
            [
                Residual_Layer(
                    create_conv_layer(
                        self.parametrization, base_channels, base_channels, ksize1=kbase1 // (powerbase**i), ksize2=kbase2 // (powerbase**i), kernel_size=kernel_size
                    ),
                    nn.ReLU(),
                )
                for i in range(1, depth)
        ...
        self._central_conv_layer = Residual_Layer(
            create_conv_layer(
                self.parametrization, base_channels, base_channels, ksize1=kbase1 // (powerbase**depth), ksize2=kbase2 // (powerbase**depth), kernel_size=kernel_size
            ),
            nn.ReLU(),
        )
        ...
        self._downsample_layers = nn.ModuleList([TrigonometricResize_2d(shape=(kbase1 // (powerbase**i), kbase2 // (powerbase**i)), upsample=False) for i in range(1, depth + 1)])

        self._upsample_layers = nn.ModuleList(
            [TrigonometricResize_2d(shape=(kbase1 // (powerbase**i), kbase2 // (powerbase**i)), downsample=False) for i in range(depth - 1, 0, -1)]
        )
    ...
    def convert_to_spectral(self):
        ...
        for i in range(1, self._depth):
            new_model._down_conv_layers[i - 1].linear_layer = gen_from_Conv2d(
                self._down_conv_layers[i - 1].linear_layer,
                ksize1=self.kbase1 // (self.powerbase ** (i * self._u_shape)),
                ksize2=self.kbase2 // (self.powerbase ** (i * self._u_shape)),
            )

        new_model._central_conv_layer.linear_layer = gen_from_Conv2d(
            self._central_conv_layer.linear_layer,
        ...
        for i in range(self._depth - 1, 0, -1):
            new_model._up_conv_layers[-i].linear_layer = gen_from_Conv2d(
                self._up_conv_layers[-i].linear_layer, ksize1=self.kbase1 // (self.powerbase ** (i * self._u_shape)), ksize2=self.kbase2 // (self.powerbase ** (i * self._u_shape))
            )

        new_model._last_block[0].linear_layer = gen_from_Conv2d(self._last_block[0].linear_layer, ksize1=self.kbase1, ksize2=self.kbase2)
...
class SpectralUNet(MultiDimUNet):
    def __init__(
            ...
            [
                nn.Sequential(
                    create_conv_layer(
                        ...
                        base_channels * (2 ** (i - 1)),
                        base_channels * (2**i),
                        ksize1=kbase1 // (powerbase**i),
                        ksize2=kbase2 // (powerbase**i),
        ...
        self._central_conv_layer = nn.Sequential(
            create_conv_layer(
                parametrization,
                base_channels * (2 ** (depth - 1)),
                base_channels * (2 ** (depth - 1)),
                ksize1=kbase1 // (powerbase**depth),
                ksize2=kbase2 // (powerbase**depth),
                kernel_size=kernel_size,
            ),
            nn.ReLU(),
        )
    ...
    def convert_to_spectral(self):
        ...
        for i in range(1, self._depth):
            cast(nn.ModuleList, new_model._down_conv_layers[i - 1])[0] = gen_from_Conv2d(
                cast(nn.ModuleList, self._down_conv_layers[i - 1])[0],
                ksize1=self.kbase1 // (self.powerbase ** (i * self._u_shape)),
                ksize2=self.kbase2 // (self.powerbase ** (i * self._u_shape)),
            )

        new_model._central_conv_layer[0] = gen_from_Conv2d(
            self._central_conv_layer[0],
            ksize1=self.kbase1 // (self.powerbase ** (self._depth * self._u_shape)),
            ksize2=self.kbase2 // (self.powerbase ** (self._depth * self._u_shape)),
        )

        for i in range(self._depth - 1, 0, -1):
            cast(nn.ModuleList, new_model._up_conv_layers[-i])[0] = gen_from_Conv2d(
                cast(nn.ModuleList, self._up_conv_layers[-i])[0],
                ksize1=self.kbase1 // (self.powerbase ** (i * self._u_shape)),
                ksize2=self.kbase2 // (self.powerbase ** (i * self._u_shape)),
            )

        new_model._last_block[0] = gen_from_Conv2d(self._last_block[0], ksize1=self.kbase1, ksize2=self.kbase2)
