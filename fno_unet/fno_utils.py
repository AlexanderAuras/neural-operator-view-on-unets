...
class SpectralConv2d_memory(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ksize1 = 1, ksize2 = 1, norm = 'forward'):
        super(SpectralConv2d_memory, self).__init__()
    ...
    def forward(self, x):
        ...
        x_ft_padded =   symmetric_padding(
                            ...
                            x_shape, compute_shape + (1-compute_shape%2)
                        )
        ...
        output = fft.irfft2(
                    irfftshift(
                        symmetric_padding(
                            ...
                            compute_shape + (1-compute_shape%2), x_shape
                        )
                    ),
                    norm = self.norm,\
                    s=tuple(x_shape)
